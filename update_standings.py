...
def get_season_data(matches, standings):
    cursor = db.cursor()
...
def get_teams(comp):
    ...
    cursor.execute(h_team_query % comp)
    teams = cursor.fetchall()
    h_teams = [t[0] for t in teams]
    cursor.execute(a_team_query % comp)
    teams = cursor.fetchall()
    a_teams = [t[0] for t in teams]

    teams = a_teams + h_teams
...
def update_comp(comp):
    ...
    fetch_and_update(matches, teams, comp, standings)
    print(standings)
